// Главный контейнер формы. Управляет активным режимом (mode), строит default values
// и Zod-схему через formAdapter, передаёт форму дочерним компонентам через FormProvider.
// useWatch следит за полем eventId - если оно заполнено, показывает карточку мероприятия.
// При смене режима форма сбрасывается через useEffect.
// ScrollArea включается только если в режиме больше 6 полей.
